...
10. `10-host_predict_on_sus_ind` - Trains a host random forest model on all the Definite/No Evidence samples, using the genes selected from all those samples, and then classifies the "Suspected" and "Indeterminate" LRTI samples with it.
11. `11-integrated_predict_on_sus_ind` - Trains a integrated logtistic regression model on all the Definite/No Evidence samples using the host probabilities and microbial scores, and then classifies the Suspected/Indeterminate samples with it.
