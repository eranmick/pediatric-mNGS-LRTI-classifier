...
9. `09-output_cv_auc_roc.R` - Generates a table with per-fold AUC values and an ROC curve from the out-of-fold probabilities of the Definite/No Evidence samples based on i) the host random forest classifier and ii) the logistic host+microbial classifier.
