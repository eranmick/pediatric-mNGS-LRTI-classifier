...
8. `08-integrated_classifier_cv.R` - Trains a logistic regression model that integrates the i) host LRTI probability, ii) viral score, and iii) bacterial/fungal score of the Definite/No Evidence samples for each train/test split, and then generates out-of-fold LRTI probabilities.
